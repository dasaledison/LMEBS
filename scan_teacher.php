...
$page_title = 'Scan Teacher';
require_once('includes/load.php');
// Check what level user has permission to view this page
page_require_level(1);

// Check if a teacher ID is scanned
if (isset($_GET['employee-id'])) {
    ...
    $teacher = find_teacher_by_id($employee_id);
    if ($teacher) {
        // teacher found, proceed to next page
        ...
        // Redirect to input_borrow_details.php with teacher ID and scanned items
        ?>
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                var url = "input_borrow_details_teacher.php";
                var separator = "?";
                var teacherId = "<?php echo $employee_id; ?>";
                var scannedItems = <?php echo json_encode($scanned_items); ?>;
                var queryParams = [];

                // Add teacher ID to the query parameters
                queryParams.push("employee-id=" + teacherId);
        ...
        $session->msg('d', "Teacher ID not found.");
...
<div class="row">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <strong>
                    ...
                    <span>Scan Teacher ID</span>
                </strong>
                <form id="student-form" class="pull-right">
                    <button type="button" onclick="submitStudentForm()" class="btn btn-xs btn-success">Scan Student ID</button>
                </form>
                ...
                <script>
                    function submitStudentForm() {
                        ...
                        window.location.href = "scan_student.php" + queryString;
                    }
                </script>
            </div>
            ...
            <div class="panel-body">
                ...
                <script type="text/javascript">
                    ...
                    $("#borrow-form").submit(function(event) {
                        ...
                        // Get the teacher ID from the input field
                        var teacherId = $("#employee-id").val().trim();
                        ...
                        // Construct the URL with teacher ID and existing query string
                        var url = 'input_borrow_details_teacher.php?employee-id=' + teacherId + '&' + queryString;

                        // Redirect to input_borrow_details.php with teacher ID and existing query string
                        location.href = url;
                    });
    ...
    $.ajax({
        url: 'check_teacher.php',
        method: 'GET',
        data: { employee_id: decodedText },
        dataType: 'json',
        success: function(response) {
            if (response.exists) {
                // Student exists, proceed with redirection
                var queryString = window.location.search.substring(1);
                var redirectUrl = 'input_borrow_details_teacher.php?employee-id=' + decodedText + '&' + queryString;
        ...
        error: function(xhr, status, error) {
            ...
            alert("An error occurred while checking the teacher. Please try again.");
        }
    });
}
                ...
                <form id="borrow-form" method="get" action="" class="clearfix">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="employee-id" name="employee-id" placeholder="Teacher ID" required>
                        </div>
                    </div>
                </form>
                <script type="text/javascript">
                    ...
                    $("#borrow-form").submit(function(event) {
                        ...
                        // AJAX request to check if student exists
        $.ajax({
            url: 'check_teacher.php', // Change to the appropriate PHP file that checks student existence
            method: 'GET',
            data: {employee_id: teacherId},
            dataType: 'json',
            success: function(response) {
                if (response.exists) {
