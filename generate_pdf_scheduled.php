...
if(isset($_GET['report']) && $_GET['report'] === 'scheduled') {
 ...
 $pdf->SetTitle('All Pending Reservations PDF Report');
 ...
 $pdf->SetY($pdf->GetY() - 1); // Move the Y position up by the height of the previous cell
 $pdf->Cell(0, 10, 'Filter: All Scheduled Reservations', 0, 1, 'L');

 // Styling for the header "All Pending Reservations"
 $pdf->SetFillColor(50, 66, 145); // Blue background color
 $pdf->SetTextColor(255, 255, 255); // White text color
 $pdf->SetFont('helvetica', 'B', 17);

 // Output header for All Pending Reservations
 $pdf->Cell(0, 10, 'All Scheduled Reservations', 0, 1, 'C', 1);


    // Fetch scheduled reservations data from the database
    $query = "SELECT borrow_id, school_id, borrowed_from, borrowed_till, subject_id, quantity, asset_num FROM all_borrowers WHERE status = 'Scheduled'";
    ...
    $pdf->Cell(40, 10, 'School ID', 1, 0, 'C', 1);
    $pdf->Cell(40, 10, 'Borrowed From', 1, 0, 'C', 1);
    $pdf->Cell(40, 10, 'Borrowed Till', 1, 0, 'C', 1);
    $pdf->Cell(20, 10, 'Quantity', 1, 0, 'C', 1);
    $pdf->Cell(50, 10, 'Asset Number', 1, 1, 'C', 1);

    // Output data for Scheduled Reservations
    while ($row = $db->fetch_assoc($result)) {
        $pdf->SetFillColor(230); // Light grey background color
        $pdf->SetTextColor(0, 0, 0); // Black text color
        $pdf->SetFont('helvetica', '', 8);
        $pdf->Cell(40, 10, $row['school_id'], 1, 0, 'C');
        $pdf->Cell(40, 10, $row['borrowed_from'], 1, 0, 'C');
        $pdf->Cell(40, 10, $row['borrowed_till'], 1, 0, 'C');
        $pdf->Cell(20, 10, $row['quantity'], 1, 0, 'C');
        $pdf->Cell(50, 10, $row['asset_num'], 1, 1, 'C');
    }

    // Output the PDF as an attachment
    ob_end_clean();
    $pdf->Output('scheduled_reservations_report.pdf', 'D');
} else {
