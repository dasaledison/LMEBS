...
if (isset($_GET['report'])) {
    ...
    $pdf->Cell(0, 10, 'Confirmed Borrower Damage Reports', 0, 1, 'C', 1);
    ...
    while ($report_borrower = $db->fetch_assoc($result_borrower)) {
        ...
        $pdf->SetTextColor(0, 0, 0); // Black text color
        $pdf->SetFont('helvetica', '', 8);
        $pdf->Cell(40, 10, $report_borrower['report_id'], 1, 0, 'C'); // Print picture path or name
        $pdf->Cell(30, 10, $report_borrower['school_id'], 1, 0, 'C'); // Print picture path or name

        $pdf->Cell(20, 10, $report_borrower['cause'], 1, 0, 'C');
        $pdf->Cell(60, 10, $report_borrower['notes'], 1, 0, 'C');
    ...
    ob_end_clean();
    $pdf->Output('confirmed_borrower_reports.pdf', 'D');
} else {
