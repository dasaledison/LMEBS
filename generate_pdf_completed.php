...
if(isset($_GET['report']) && $_GET['report'] === 'completed') {
    ...
    $pdf->SetTitle('All Completed Reservations PDF Report');
    ...
    $pdf->SetY($pdf->GetY() - 1); // Move the Y position up by the height of the previous cell
    $pdf->Cell(0, 10, 'Filter: All Completed Reservations', 0, 1, 'L');

    // Styling for the header "All Completed Reservations"
    $pdf->SetFillColor(50, 66, 145); // Blue background color
    $pdf->SetTextColor(255, 255, 255); // White text color
    $pdf->SetFont('helvetica', 'B', 17);

    // Output header for All Completed Reservations
    $pdf->Cell(0, 10, 'All Completed Reservations', 0, 1, 'C', 1);

    // Fetch completed reservations data from the database
    $query = "SELECT borrow_id, school_id, borrowed_from, borrowed_till, quantity, asset_num FROM all_borrowers WHERE status = 'Completed'";
    ...
    ob_end_clean();
    $pdf->Output('completed_reservations_report.pdf', 'D');
} else {
