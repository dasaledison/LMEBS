...
if ($user) {
    ...
    if ($user_type === 'teacher') {
        // Fetch 'Confirmed' damaged reports for the current teacher user
        $status = 'Confirmed'; // Set the status to Confirmed
        $statusClass = 'status-' . strtolower(str_replace(' ', '-', $status));
        $damaged_reports = find_by_sql("SELECT * FROM teachers_damaged_reports WHERE user_id = {$current_user_id} AND report_status = '{$status}'");
        ...
        // Display a message for non-teacher users
        $message = "You are not a teacher. No reports available.";
    }
} else {
    // User not found
    echo "User not found.";
