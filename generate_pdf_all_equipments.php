...
if(isset($_GET['report'])) {
    ...
    $pdf->SetTitle('All Equipments PDF Report');
    ...
    // Styling for the header "All Equipments"
    $pdf->SetFillColor(50, 66, 145); // Blue background color
    $pdf->SetTextColor(255, 255, 255); // White text color
    $pdf->SetFont('helvetica', 'B', 17);

    // Output header for All Equipments
    $pdf->Cell(0, 10, 'All Equipments', 0, 1, 'C', 1);
    ...
    foreach ($products as $index => $product) {
        ...
        $pdf->Cell(10, 10, $product['quantity'], 1, 0, 'C');
        $pdf->Cell(10, 10, $product['in_use_qty'], 1, 0, 'C');
        $pdf->Cell(20, 10, $product['item_num'], 1, 0, 'C');
        $pdf->Cell(30, 10, $product['asset_num'], 1, 0, 'C');
        $pdf->Cell(20, 10, $product['brand'], 1, 0, 'C');
        $pdf->Cell(10, 10, $product['location'], 1, 0, 'C');
        $pdf->Cell(20, 10, $product['description'], 1, 0, 'C');
        $pdf->Cell(40, 10, $product['equipment_barcode'], 1, 1, 'C');
    }
