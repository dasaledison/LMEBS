            ...
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <strong>
                        ...
                        <span>Edit Other Damage Report</span>
                    </strong>
                </div>
                <div class="panel-body">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="text-center">Edit Other Damage Report</h3>
                        </div>
                        <div class="card-body">
                            <!-- Edit report form -->
                            <form id="editReportForm" action="edit_process_other_damage_report.php" method="post"
                                enctype="multipart/form-data">
                                <input type="hidden" name="report_id" value="<?php echo $report['report_id']; ?>">
                                <div class="form-group">
                                    <label for="product_name">Product Name:</label>
                                    <input type="text" class="form-control" name="product_name" id="product_name"
                                        value="<?php echo remove_junk($report['product_name']); ?>" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="asset_num">Asset Number:</label>
                                    <input type="text" class="form-control" name="asset_num" id="asset_num"
                                        value="<?php echo remove_junk($report['asset_num']); ?>" readonly>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="quantity" id="quantity"
                                        value="<?php echo remove_junk($report['quantity']); ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="date_of_incident">Date of Incident:</label>
                                    <input type="date" class="form-control" name="date_of_incident"
                                        id="date_of_incident"
                                        value="<?php echo remove_junk($report['date_of_incident']); ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="cause">Cause of Damage:</label>
                                    <textarea class="form-control" name="cause" id="cause"
                                        rows="3"
                                        required><?php echo remove_junk($report['cause']); ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="notes">Notes:</label>
                                    <textarea class="form-control" name="notes" id="notes"
                                        rows="3"><?php echo remove_junk($report['notes']); ?></textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <br>
                                        <img src="<?php echo remove_junk($report['product_img']); ?>"
                                            class="img img-thumbnail" alt="Product Image">
                                    </div>
                                <?php endif; ?>
                                  
                                    <input type="file" class="custom-file-input custom-file-label form-control-file"
                                        name="product_img" id="product_img">
                                    <label class="custom-file-label" for="product_img"></label>
                                </div>
                                <div class="form-group">
                                    <label for="report_status">Report Status:</label>
                                    <select class="form-control" name="report_status" id="report_status" required>
                                        <option value="Under Review"
                                            <?php echo ($report['report_status'] == 'Under Review') ? 'selected' : ''; ?>>
                                            Under Review
                                        </option>
                                        <option value="Confirmed"
                                            <?php echo ($report['report_status'] == 'Confirmed') ? 'selected' : ''; ?>>
                                            Confirmed
                                        </option>
                                        <option value="Resolved"
                                            <?php echo ($report['report_status'] == 'Resolved') ? 'selected' : ''; ?>>
                                            Resolved
                                        </option>
                                    </select>
                                </div>
